...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        # for every element that should be in out storage
        for i in range(len(out)):
            # calculate the index of the output element
            to_index(i, out_shape, out_index)
            # convert from output index back to the input index corresponding to it
            broadcast_index(out_index, out_shape, in_shape, in_index)
            # get the argument to pass into the function from the in_storage
            data = in_storage[index_to_position(in_index, in_strides)]
            # apply function to argument and store it in the correct position in out storage
...
def tensor_reduce(fn: Callable[[float, float], float]) -> Any:
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        out_index = np.array(out_shape)
        red_dim_index = np.array(reduce_shape)

        for i in range(len(out)):
            to_index(i, out_shape, out_index)
            j = index_to_position(out_index, out_strides)
            for k in range(a_shape[reduce_dim]):
                to_index(k, reduce_shape, red_dim_index)
                a_index = out_index + red_dim_index
                out[j] = fn(out[j], a_storage[index_to_position(a_index, a_strides)])
