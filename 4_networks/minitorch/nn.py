...
def tile(input: Tensor, kernel: Tuple[int, int]) -> Tuple[Tensor, int, int]:
    ...
    h = height // kh
    w = width // kw
    res = input.contiguous().view(batch, channel, height, w, kw)
    res = res.permute(0, 1, 3, 2, 4)
    res = res.contiguous().view(batch, channel, h, w, kh * kw)
    return res, h, w
...
def avgpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    # TODO: Implement for Task 4.3.
    # raise NotImplementedError("Need to implement for Task 4.3")
    tiled_input, h, w = tile(input, kernel)
    res = tiled_input.mean(dim=4)
    return res.view(batch, channel, h, w)
